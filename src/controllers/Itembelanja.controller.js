...
module.exports = {
  ...
  create: (req, res) => {
    ...
    db.Itembelanja.insert(req.body, result => {
  ...
  getSome: (req, res) => {
    ...
    console.log("Itembelanja",condition);

    console.log(condition);
    //  selectSome: (condition,range,sort,cb) => {
    db.Itembelanja.selectSome(condition,limit,sort, data => {
      res.json(data)
    });
  ...
  createNew: (req, res) => {
    db.Itembelanja.insertOne(req.body.vals, result => {
      res.json({ id: result.insertId })
    })
  },
  getById: (req, res) => {
    console.log("==getById==");
    db.Itembelanja.selectOne(req.params.id, data => {
      res.json(data)
    })
  },
  updateById: (req, res) => {
    //req.body.registration_expiry_date = new Date(req.body.registration_expiry_date).toISOString().slice(0, 10);
    db.Itembelanja.updateOne(req.body, req.body.id, result => {
  ...
  deleteSome: (req, res) => {
    ...
    db.Itembelanja.deleteSome(condition, data => {
      res.json(data)
    })
  },
  
  deleteById: (req, res) => {
    db.Itembelanja.deleteOne(req.params.id, data => {
      res.json(data)
    })
  }
}
